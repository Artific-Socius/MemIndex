...
The MemIndex Benchmark is an enhanced evaluation framework for assessing the memory capabilities of Large Language Models, built upon and refined from the original LTM Benchmark \ph{[ref]}.
...
\item Memory Span Stress Testing \ph{[ref\_ltm]}: A fixed Memory Span is maintained, ensuring that a substantial volume of "noise" (distractor dialogue) is inserted between the critical data points and the query. This is designed to rigorously challenge the model's long-term retrieval and retention capabilities.
...
The dataset under consideration is derived from the GoodAI LTM Benchmark \ph{[ref]}. A comprehensive architectural system has been implemented for the purpose of reconstructing the original data. A dedicated Markup Language was developed for the purpose of facilitating rapid dataset editing. This Markup Language is subsequently processed by the proprietary Dataset Compiler to generate the final JSON-formatted evaluation sets.
